...
const TaskDetails = (props) => {
  ...
  const updateTask = async () => {
    ...
    if (!values.title) {
      ...
      const newT = {
        ...values,
        assignee: getValues("assignee")?.id,
        group: team.id,
      };
      try {
  ...
  const listTeamMembers = () => {
    return team.members.map((m, i) =>
      m.user_name === asg?.user_name ? (
        <MenuItem key={i} selected value={m}>
          {m.user_name}
        </MenuItem>
      ) : (
        ...
        <AccordionActions>
          {!task.is_done && !editMode && (
            <Button
              ...
              color="success"
              size="small"
              onClick={() => notifyMarkedAsDone(task)}
            >
              Mark as done
            </Button>
          )}
