...
const SearchBar = (props) => {
      ...
      case SORT_BY["ASSIGNEE"]:
        t.sort((a, b) => {
          ...
          return a.group === b.group &&
            getUsernameByIDTeamId(a.assignee, a.group)?.toLowerCase() >
              getUsernameByIDTeamId(b.assignee, b.group)?.toLowerCase()
            ? 1
            : -1;
        });
        break;
  ...
  const handleAssigneeFilter = (e) => {
    ...
    t = t.filter((task) => {
      if (task.group === watchTeam) return task.assignee === assignee;
      return 1;
    });
    setTasks(t);
  };
